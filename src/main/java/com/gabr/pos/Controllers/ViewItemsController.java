...
import com.gabr.pos.PDF.ItemsBarCodeTemplete;
import com.gabr.pos.PDF.ItemsBarCodeTempleteOne;
import com.gabr.pos.PDF.ItemsReprot;
...
public class ViewItemsController implements Initializable {
    ...
    @FXML
    private HBox storeTotal;

    int SHOWADVANCED = 0;
    @FXML
    private TableColumn<String , String> updatedDate;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        try {
            ...
            Image img = new Image(getClass().getResourceAsStream(EXCEL_ICON));

            excel_icon.setImage(img);


            // check to show advanced view
            SHOWADVANCED = showAdvanced();
            if (SHOWADVANCED == 0) {
                storeTotal.setVisible(false);
                totalMony.setVisible(false);
            } else {
                storeTotal.setVisible(true);
...
//            warning.setFocusTraversable(false);
        ...
        itemsfileLocation = REPORT_MAIN_FOLDER + ITEMS_FOLDER + "//";
        itemsfileLocation_Barcode = REPORT_MAIN_FOLDER + ITEMS_BARCODE_FOLDER + "//";
    ...
    private void refresh() {
        ...
        try {
            ...
            query = "select items.id , items.code , items.name , items.amount , items.price , items.priceforcustomer ,items.lastUpdatedDate , store.storeName from items , store where items.storeCode = store.id ";
    ...
    private void loadDate() {
        ...
        try {
            ...
            updatedDate.setCellValueFactory(new PropertyValueFactory<>("lastUpdatedDate"));
            ...
            updatedDate.setStyle("-fx-alignment: CENTER;");
                    ...
                    public void updateItem(String item, boolean empty) {
                        ...
                        if (empty) {
                            ...
                            editIcon.setOnMouseClicked((MouseEvent event) -> {
                                ...
                                itm = customerTable.getSelectionModel().getSelectedItem();
                            //    openEditItemPage(itm);
                                OPEN_EDIT_ITEM_PAGE(itm);
                            ...
                            print.setOnMouseClicked((MouseEvent event) -> {
                                ...
                                try {
                                    ...
                                    LOG_EXCEP(this.getClass().getName(), "loadData Print barcode", ex);
                                }
            ...
            LOG_EXCEP(this.getClass().getName(), "loadData", ex);
        }
    ...
    public void loadDataByAmount() {
        ...
        try {
            ...
            query = "SELECT items.id , items.code , items.name , items.amount , items.price , items.priceforcustomer,items.lastUpdatedDate , store.storeName from items , store where items.storeCode=store.id AND items.amount <" + STORE_ALARM;
            ...
            LOG_EXCEP(this.getClass().getName(), "loadDataByAmount", ex);
        }
    ...
    public int checkitem() {
        ...
        try {
            ...
            LOG_EXCEP(this.getClass().getName(), "checkitem", ex);
        }
    ...
    private void barcodeSearch(KeyEvent event) {
        ...
        try {
            ...
            query = "SELECT items.id , items.code , items.name , items.amount , items.price , items.priceforcustomer , items.lastUpdatedDate , store.storeName from items , store where items.storeCode=store.id AND items.code = ?";
            preparedStatement = connection.prepareStatement(query);
            ...
            LOG_EXCEP(this.getClass().getName(), "barcodeSearch", ex);
        }
        calcAllTotal();
    ...
    private void loadItemByStore(int s) {
        ...
        try {
            ...
            query = "SELECT items.id , items.code , items.name , items.amount , items.price , items.priceforcustomer ,items.lastUpdatedDate , store.storeName from items , store where items.storeCode=store.id AND items.storeCode=?";
            preparedStatement = connection.prepareStatement(query);
            ...
            LOG_EXCEP(this.getClass().getName(), "loadItemByStore", ex);
        }
    }
    ...
    private void printBarcode(MouseEvent event) {
        ...
        try {
            ...
            LOG_EXCEP(this.getClass().getName(), "printBarcode", ex);
        }
    ...
    private void updateAll(MouseEvent event) {
        ...
        openEditStoreSalesPage(storesArray);
        /*
...
//        } catch (Exception ex) {
//            LOG_EXCEP(this.getClass().getName(), "printExcel", ex);
//        }
    }
    ...
    public void openEditStoreSalesPage(ArrayList<String> stores) {
        try {
            FXMLLoader loader = new FXMLLoader();
            loader.setLocation(getClass().getResource("/screen/editStoreSales.fxml"));
            loader.load();

            EditStoreSales editStoreSales = loader.getController();
            editStoreSales.setTextField(stores);
            ...
            LOG_EXCEP(this.getClass().getName(), "openEditStoreSalesPage", ex);
        }
    }
    ...
    public int showAdvanced() {

        int code = 0;
        try {

            connection = DbConnect.getConnect();
            query = "SELECT storePrice FROM `showadvanced` ";
            preparedStatement = connection.prepareStatement(query);
            resultSet = preparedStatement.executeQuery();

            while (resultSet.next()) {

                code = resultSet.getInt("storePrice");
